...
/*   Updated: 2023/11/24 20:57:34 by junkim2          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/so_long_bonus.h"

static void	set_sprite_image(t_mlx *mlx)
{
	int			i;
	int			j;
	char		tmp[30];
...
void	set_image(t_mlx *mlx)
{
	...
	mlx->imgpack.exit = mlx_xpm_file_to_image(mlx->mlx_ptr, "images/exit.xpm", \
		&mlx->imgpack.width, &mlx->imgpack.height);
	mlx->imgpack.enemy = mlx_xpm_file_to_image(mlx->mlx_ptr, \
	"images/robot_cleaner.xpm", &mlx->imgpack.width, &mlx->imgpack.height);
